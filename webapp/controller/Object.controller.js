sap.ui.define([
    ...
    form: function( oControlEvent ) {
      let value = oControlEvent.getParameter("value");
      let pref = value.substr(0,3);
      if( pref !== "HT-" ){
        sap.m.MessageToast.show("Il Product ID deve contenere il prefisso HT-");
        this.byId("productID").setValue("HT-");
      } else {
        let sub = value.substr(3,7);
        let regex = /^[0-9]*$/;
        if( sub.length !== 4 ){
          this.byId("productID").setValue(pref + sub.slice(0,4));
          this.byId("checkID").setEnabled(false);
        } else if( !sub.match(regex) ){
          this.byId("productID").setValue(pref);
          sap.m.MessageToast.show("L'ID del prodotto deve essere numerico");
          this.byId("checkID").setEnabled(false);
        } else {
    ...
    check: function() {
      ...
      this.getModel().read("/ProductSet('" + productID + "')", {
        success: () => {
          sap.m.MessageToast.show("Prodotto esistente");
        },
        error: () => {
